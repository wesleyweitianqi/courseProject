...
exports.allCoursesController = (req, res) => {
  ...
  page = (page - 1) * size;
  const pageSql =
    "select * from course where del = 0 and category=? order by id limit ?, ?";
  const totalSql =
    "select count(*) as total from course where del=0 and category =?";
...
exports.searchCourseController = (req, res) => {
  let { title } = req.query;
  const searchSql =
    "select * from course where del = 0 and title like ?";
  const totalSql =
    "select count(*) from course where del = 0 and title like ?";
  const values  = [ `%${title}%` ];
  db.query(searchSql, values, (err, resSearch) => {
    ...
    db.query(totalSql, title, (err, resTotal) => {
      ...
      res.send({
        code: 0,
        data: { list: resSearch, total: resTotal[0].total },
      });
    });
  });
};
...
exports.deleteCourseController = (req, res) => {
  ...
  db.query(sql, id, (err, results) => {
    if (err) return res.send({ code: 1, message: err.message });
    res.send({ code: 0, message: "delete successfully" });
  });
};
